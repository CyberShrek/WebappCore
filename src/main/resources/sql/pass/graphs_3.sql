 ...
 monthly_base AS (
     SELECT year, month,
            ...
            sum(correspon.sumbil + correspon.sumpl  + correspon.sumserv  + correspon.sumkomsb
                + correspon.sumbilv + correspon.sumplv + correspon.sumservv + correspon.sumkomsbv) / 1000  AS monthly_income
...
SELECT month_date,
       round(monthly_income) AS "Доходные поступления",
       round(avg(monthly_income) OVER (PARTITION BY month_date IS NOT NULL)) AS "Средний показатель"
