WITH day_series AS (
 ...
 daily_base as (
     SELECT
         ...
         sum(correspon.kolpm   - correspon.kolvozm)                                                         AS daily_passengers
...
SELECT formatted_day_date,
       daily_passengers AS "Количество пассажиров",
       round(avg(daily_passengers) OVER (PARTITION BY formatted_day_date IS NOT NULL)) AS "Средний показатель"
